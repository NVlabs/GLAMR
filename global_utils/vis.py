...
def images_to_video(img_dir, out_path, img_fmt="%06d.png", fps=30, crf=25, verbose=True):
    ...
    cmd = ['/usr/bin/ffmpeg', '-y', '-r', f'{fps}', '-f', 'image2', '-start_number', '0',
...
def hstack_videos(video1_path, video2_path, out_path, crf=25, verbose=True, text1=None, text2=None, text_color='white', text_size=60):
    ...
    os.makedirs(osp.dirname(out_path), exist_ok=True)
    cmd = ['/usr/bin/ffmpeg', '-y', '-i', video1_path, '-i', video2_path, '-filter_complex', 'hstack,format=yuv420p', 
    ...
    if write_text:
        font_file = 'C\\:/Windows/Fonts/arial.ttf' if platform.system() == 'Windows' else '/usr/share/fonts/truetype/lato/Lato-Regular.ttf'
        ...
        cmd = ['/usr/bin/ffmpeg', '-i', tmp_file, '-y', '-vf', draw_str, '-c:a', 'copy', out_path]
        if not verbose:
            cmd += ['-hide_banner', '-loglevel', 'error']
        subprocess.run(cmd)
        os.remove(tmp_file)
...
def vstack_videos(video1_path, video2_path, out_path, crf=25, verbose=True, text1=None, text2=None, text_color='white', text_size=60):
    ...
    os.makedirs(osp.dirname(out_path), exist_ok=True)
    cmd = ['/usr/bin/ffmpeg', '-y', '-i', video1_path, '-i', video2_path, '-filter_complex', 'vstack,format=yuv420p', 
           '-vcodec', 'libx264', '-crf', f'{crf}', tmp_file if write_text else out_path]
    if not verbose:
        cmd += ['-hide_banner', '-loglevel', 'error']
    subprocess.run(cmd)
